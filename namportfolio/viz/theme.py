...
コーポレートカラー ``#c80000`` を基調にしている。識別色の 1 番目がこの赤、順序・量の
ランプはこの赤を軸にした濃淡、極性は**正が赤・負が青**（日本の株式慣行）。海外向けに
反転するなら ``theme.POSITIVE_HUE = "blue"``。

.. rubric:: 検証

色覚特性（P型・D型）下での識別性を検証済み。

- categorical: 両モードで全チェック通過。隣接ペアの CVD ΔE 9.1（light）/ 8.4（dark）、
  通常視 19.6 / 19.3
- ordinal: 実際に使う段階数（3・5 分位）で単調な明度差と背景コントラストを満たす。
  7 段階以上では明度差が詰まる（単一色相ランプの原理的な制約）
...
_CATEGORICAL = {
    ...
    # dark では #c80000 が背景に対して 2.87:1 しか取れないので、
    # 同じ色相で明度を上げた段階を使う
    "dark": [
        "#d41414",
        "#3987e5",
...
# 順序用の単一色相ランプ。コーポレートカラーを軸に明度を振っている。
# light 端・dark 端はいずれも背景から十分離れた段階から始める。
_ORDINAL = {
    "light": [
        "#ee9797",
        "#e88484",
...
_SEQUENTIAL = {
    "light": ["#fbe0e0", "#ee9797", "#d84a4a", "#ac0000", "#5a0000"],
    "dark": ["#5a0000", "#ac0000", "#d84a4a", "#ee9797", "#fbe0e0"],
}
