...
**赤基調。** コーポレートカラー `#c80000` が軸（dark では明度を上げた `#d41414`）。
極性は正が赤・負が青（日本の株式慣行）。

**分位を categorical で塗らない**（順序が色から読めなくなる）。縦軸を 2 本にしない。
**色を差し替えるときは色覚特性下での識別性を再検証すること**（dataviz スキルの
`scripts/validate_palette.js`）。見た目で決めない。

plotly はスタイルヘルパー（`viz/plotly.py`）だけ。**図は二重実装しない**。
plotly express は trace に色を埋め込むので `layout.colorway` では上書きできず、
`apply_theme` が trace 側を塗り替えている。
